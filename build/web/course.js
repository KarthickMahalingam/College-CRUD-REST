...
// Model for Courses entity
    models.Courses = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/CollegeRecords/webresources/entity.courses/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('courseId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8084/CollegeRecords/webresources/entity.courses/';
            }
    ...
    // Collection class for Courses entities
    models.CoursesCollection = Backbone.Collection.extend({
        model: models.Courses,
        url: "http://localhost:8084/CollegeRecords/webresources/entity.courses/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CoursesCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-courses-list-itemis template identifier for item
                        templateName: '#tpl-courses-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.courses = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.courses,
                    // tpl-courses-details is template identifier for chosen model element
                    templateName: '#tpl-courses-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Courses();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-courses-details is a template identifier for chosen model element
                templateName: '#tpl-courses-details',
