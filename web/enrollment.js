...
// Model for Enrollments entity
    models.Enrollments = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/CollegeRecords/webresources/entity.enrollments/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('enrollId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8084/CollegeRecords/webresources/entity.enrollments/';
            }
    ...
    // Collection class for Enrollments entities
    models.EnrollmentsCollection = Backbone.Collection.extend({
        model: models.Enrollments,
        url: "http://localhost:8084/CollegeRecords/webresources/entity.enrollments/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.EnrollmentsCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-enrollments-list-itemis template identifier for item
                        templateName: '#tpl-enrollments-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.enrollments = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.enrollments,
                    // tpl-enrollments-details is template identifier for chosen model element
                    templateName: '#tpl-enrollments-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Enrollments();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-enrollments-details is a template identifier for chosen model element
                templateName: '#tpl-enrollments-details',
