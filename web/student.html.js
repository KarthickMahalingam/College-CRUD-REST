...
// Model for Students entity
    models.Students = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/CollegeRecords/webresources/entity.students/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('studentid');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8084/CollegeRecords/webresources/entity.students/';
            }
    ...
    // Collection class for Students entities
    models.StudentsCollection = Backbone.Collection.extend({
        model: models.Students,
        url: "http://localhost:8084/CollegeRecords/webresources/entity.students/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.StudentsCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-students-list-itemis template identifier for item
                        templateName: '#tpl-students-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.students = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.students,
                    // tpl-students-details is template identifier for chosen model element
                    templateName: '#tpl-students-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Students();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-students-details is a template identifier for chosen model element
                templateName: '#tpl-students-details',
